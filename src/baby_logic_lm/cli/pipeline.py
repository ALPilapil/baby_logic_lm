"""
cli/pipeline.py — successor to the old root-level main.py.

Runs a --tasks sequence with checkpoint chaining, N-fold repeated runs, and
train/eval/both modes -- the same argparse surface as before, but each
task's config is now built via Hydra's Compose API instead of a Python
dict lookup.

    python -m baby_logic_lm.cli.pipeline --tasks dyck_pretrain dyck_5m_childes \
        --epochs 1 --pretrain-tokens 5000000 --runs 3 --tag 10m --mode both

Must be run from the repo root (all data/model paths in the task configs
are relative, e.g. "./data/...", "./models/...").
"""

import argparse
import logging
import os
from pathlib import Path

from datasets import load_from_disk
from hydra import compose, initialize_config_dir

from baby_logic_lm.config_schema import register_configs
from baby_logic_lm.logging_utils import setup_logging
from baby_logic_lm.training.runner import run_task, run_task_eval_only, run_task_train_only

logger = logging.getLogger(__name__)

CONFIG_DIR = str(Path(__file__).resolve().parents[3] / "configs")


def avg_example_length(dataset_path, sample=1000):
    ds = load_from_disk(dataset_path)["train"]
    n = min(sample, len(ds))
    return sum(len(ex["input_ids"]) for ex in ds.select(range(n))) / n


def _discover_task_names() -> list[str]:
    return sorted(p.stem for p in (Path(CONFIG_DIR) / "task").glob("*.yaml"))


def parse_args(task_names: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train baby_logic_lm experiments.",
        epilog=(
            "Example:\n"
            "  python -m baby_logic_lm.cli.pipeline --tasks dyck_pretrain dyck_5m_childes "
            "--epochs 1 --pretrain-tokens 5000000"
        ),
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    parser.add_argument(
        "--tasks",
        nargs="+",
        required=True,
        metavar="TASK",
        help=f"Task names to run in order. Available: {task_names}.",
    )
    parser.add_argument(
        "--epochs",
        type=int,
        default=1,
        help="num_train_epochs applied to every task (default: 1).",
    )
    parser.add_argument(
        "--pretrain-tokens",
        type=int,
        default=None,
        metavar="N",
        help=(
            "Token budget for pre-training tasks. Sets train_truncation automatically: "
            "N // 512 for paren (fixed 512-token blocks); "
            "N // avg_example_length for pos (variable length)."
        ),
    )
    parser.add_argument(
        "--runs",
        type=int,
        default=1,
        metavar="N",
        help="Number of times to repeat the full task sequence (default: 1). Each run uses its index as the random seed.",
    )
    parser.add_argument(
        "--tag",
        type=str,
        default="",
        metavar="LABEL",
        help="Optional label written to every row of training_results.csv for grouping runs.",
    )
    parser.add_argument(
        "--mode",
        choices=["train", "eval", "both"],
        default="both",
        help="'train': train only (no eval/CSV); 'eval': evaluate a saved model; 'both': train then evaluate (default).",
    )
    args = parser.parse_args()

    if args.runs < 1:
        parser.error("--runs must be at least 1")
    return args


def main():
    setup_logging()
    register_configs()

    task_names = _discover_task_names()
    args = parse_args(task_names)

    for name in args.tasks:
        if name not in task_names:
            raise ValueError(f"Unknown task '{name}'. Valid tasks: {task_names}")

    with initialize_config_dir(config_dir=CONFIG_DIR, version_base=None):
        base_cfgs = {
            name: compose(config_name="config", overrides=[f"task={name}"])
            for name in set(args.tasks)
        }

        # ── Validate checkpoint dependencies before running anything ──────────
        produced_in_run = set()
        for name in args.tasks:
            task_cfg = base_cfgs[name].task
            load_path = task_cfg.model_load_path
            if load_path and load_path not in produced_in_run and not os.path.exists(load_path):
                raise FileNotFoundError(
                    f"Task '{name}' requires checkpoint '{load_path}' but it does not exist. "
                    "Run the pre-training task that produces it first."
                )
            if args.mode == "eval":
                run_paths = (
                    [f"{task_cfg.model_save_path}_run{r}" for r in range(1, args.runs + 1)]
                    if args.runs > 1
                    else [task_cfg.model_save_path]
                )
                missing = [p for p in run_paths if not os.path.exists(p)]
                if missing:
                    raise FileNotFoundError(
                        f"Task '{name}' is missing trained models: {missing}. "
                        "Run training first."
                    )
            produced_in_run.add(task_cfg.model_save_path)

        paths_produced_in_command = {base_cfgs[n].task.model_save_path for n in args.tasks}

        for run_num in range(1, args.runs + 1):
            for name in args.tasks:
                cfg = compose(config_name="config", overrides=[f"task={name}"])
                task = cfg.task

                if not task.lock_epochs:
                    task.num_train_epochs = args.epochs

                if task.is_pretrain and args.pretrain_tokens:
                    if "paren" in name:
                        task.train_truncation = args.pretrain_tokens // 512
                    else:
                        avg_len = avg_example_length(task.data_path)
                        task.train_truncation = int(args.pretrain_tokens // avg_len)

                elif task.token_limit and task.use_custom_collator:
                    # NSP/NUP examples are two short utterances + EOS tokens, not
                    # fixed 512-token blocks, so the configured train_truncation
                    # (sized assuming ~512 tokens/example) can silently under-shoot
                    # task.token_limit. Recompute it from the real average example
                    # length so each epoch uses roughly token_limit / num_train_epochs
                    # tokens as intended.
                    avg_len = avg_example_length(task.data_path)
                    per_epoch_budget = task.token_limit / task.num_train_epochs
                    dataset_size = len(load_from_disk(task.data_path)["train"])
                    task.train_truncation = min(int(per_epoch_budget // avg_len), dataset_size)

                if args.runs > 1:
                    task.model_save_path = f"{task.model_save_path}_run{run_num}"
                    if task.model_load_path and task.model_load_path in paths_produced_in_command:
                        task.model_load_path = f"{task.model_load_path}_run{run_num}"

                if args.mode == "train":
                    run_task_train_only(cfg, run_num, args.tag)
                elif args.mode == "eval":
                    run_task_eval_only(cfg, run_num, args.tag)
                else:
                    run_task(cfg, run_num, args.tag)


if __name__ == "__main__":
    main()
