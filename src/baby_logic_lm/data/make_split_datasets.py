...
Run once from the project root before training:
    ...
    python -m baby_logic_lm.data.make_split_datasets
"""
...
from baby_logic_lm.data.dataprep import make_nt_dataset, make_pair_dataset
from baby_logic_lm.data.format import make_nup_pairs
...
def find_boundaries(nt_lines, tokenizer):
    ...
    logger.info("Scanning nt_text.txt for token boundaries...")
    for i, line in enumerate(nt_lines):
        ...
        for tgt, key in list(targets.items()):
            if cumulative >= tgt and key not in boundaries:
                boundaries[key] = i
                logger.info("%s: line %s (~%s tokens)", key, f"{i:,}", f"{cumulative:,}")
                del targets[tgt]
        if not targets:
            break
    ...
    for key in list(targets.values()):
        ...
        logger.info("%s: end of file (target not reached)", key)
...
def main():
    ...
    logger.info("%s tagged lines", f"{len(tagged_lines):,}")
    ...
    logger.info("Tagged-line boundaries: 5m=%s, 10m=%s, half=%s",
                f"{tb5m:,}", f"{tb10m:,}", f"{tbhalf:,}")
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        logger.info("Creating data/split/nt_10m ...")
        make_nt_dataset(
            write_text(nt_lines[:b10m], tmp, "nt_10m"),
            f"{SPLIT_DIR}/nt_10m", tokenizer,
        )

        logger.info("Creating data/split/nt_5m_a (first 5M) ...")
        make_nt_dataset(
            write_text(nt_lines[:b5m], tmp, "nt_5m_a"),
            f"{SPLIT_DIR}/nt_5m_a", tokenizer,
        )

        logger.info("Creating data/split/nt_half_a (first ~13.2M) ...")
        make_nt_dataset(
        ...
        logger.info("Creating data/split/nsp_5m_b (second 5M) ...")
        ...
        logger.info("Creating data/split/nsp_half_b (second half) ...")
        ...
        logger.info("Creating data/split/nup_5m_b (second 5M) ...")
        ...
        logger.info("Creating data/split/nup_half_b (second half) ...")
    ...
    logger.info("All 7 split datasets saved to %s/", SPLIT_DIR)
