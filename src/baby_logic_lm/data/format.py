...
def main():
    ...
    logger.info("Wrote ./data/nt_text.txt")
    ...
    logger.info("Wrote ./data/nsp_text.jsonl (%s pairs)", f"{len(nsp_pairs):,}")

    nup_pairs = make_nup_pairs(clean_text)
    write_jsonl(nup_pairs, "./data/nup_text.jsonl")
    logger.info("Wrote ./data/nup_text.jsonl (%s pairs)", f"{len(nup_pairs):,}")
